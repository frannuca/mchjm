...
def transformDiscountToInstantaneousForward(frame:pd.DataFrame,tyearstart:float,tyearend:float,N:int):
    xnew = np.linspace(tyearstart,tyearend,N)
    xnew = np.array(list(map(lambda s: np.round(s,decimals=4),xnew)))
    x = np.array([tenor2years(t)[1] for t in  frame.columns])
    ...
    return pd.DataFrame(data=data,columns=xnew,index=frame.index)
